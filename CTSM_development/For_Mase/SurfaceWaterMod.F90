...
contains
  ...
  subroutine UpdateFracH2oSfc(bounds, num_soilc, filter_soilc, &
    ...
    call BulkDiag_FracH2oSfc(bounds, num_soilc, filter_soilc, &
         ...
         qflx_sfc_depth = b_waterflux_inst%qflx_sfc_depth(begc:endc), &
         frac_sfc = b_waterflux_inst%frac_sfc(begc:endc))
  ...
  subroutine BulkDiag_FracH2oSfc(bounds, num_soilc, filter_soilc, &
       ...
       qflx_too_small_h2osfc_to_soil,qflx_sfc_depth, frac_sfc)
    !
    ! !DESCRIPTION:
    ...
    real(r8) , intent(inout) :: qflx_sfc_depth(bounds%begc: )
    real(r8) , intent(inout) :: frac_sfc(bounds%begc: )
    ...
    SHR_ASSERT_FL((ubound(qflx_sfc_depth, 1) == bounds%endc), sourcefile, __LINE__)
    SHR_ASSERT_FL((ubound(frac_sfc, 1) == bounds%endc), sourcefile, __LINE__)
