module WaterFluxType
  ...
  type, public :: waterflux_type
     ...
     real(r8), pointer :: qflx_sfc_depth(:) 
     real(r8), pointer :: frac_sfc(:)
...
contains
  ...
  subroutine InitAllocate(this, bounds, tracer_vars)
    ...
    call AllocateVar1d(var = this%qflx_sfc_irrig_col, name = 'qflx_sfc_irrig_col', &
         container = tracer_vars, &
         bounds = bounds, subgrid_level = BOUNDS_SUBGRID_COLUMN)
    call AllocateVar1d(var = this%qflx_sfc_depth, name = 'qflx_sfc_depth', &
         container = tracer_vars, &
         bounds = bounds, subgrid_level = BOUNDS_SUBGRID_COLUMN)
    call AllocateVar1d(var = this%frac_sfc, name = 'frac_sfc', &
         container = tracer_vars, &
         bounds = bounds, subgrid_level = BOUNDS_SUBGRID_COLUMN)
  ...
  subroutine InitHistory(this, bounds)
    ...
    this%qflx_sfc_depth(begc:endc) = spval
    call hist_addfld1d ( &
         fname=this%info%fname('SFC_DEPTH'), &
         units='mm H2O', &
         avgflag='A', &
         long_name=this%info%lname('surface water depth'), &
         ptr_col=this%qflx_sfc_depth)
    this%frac_sfc(begc:endc) = spval
    call hist_addfld1d ( &
         fname=this%info%fname('FRAC_SFC'), &
         units='%', &
         avgflag='A', &
         long_name=this%info%lname('fraction of surface water'), &
         ptr_col=this%frac_sfc)
  ...
  subroutine InitCold(this, bounds)
    ...
    this%qflx_sfc_depth (bounds%begc:bounds%endc) = 0.0_r8
    this%frac_sfc (bounds%begc:bounds%endc) = 0.0_r8
